/**
 * msg_OM.cc
 * Copyright (C) 2013 Seven <gergo_f@yahoo.com>
 *
 ...
 * USA.
 *
 * $Id: msg_OM.cc,v 1.00 2013/01/09 10:44:37 Seven Exp $
 */
...
RCSTAG( "$Id: msg_OM.cc,v 1.00 2013/01/09 10:44:37 Seven Exp $" ) ;
...
class msg_OM : public ServerCommandHandler
{
public:
	msg_OM( xServer* theServer )
...
// i OM #3dx +o eAA
// J[K M DEMET_33 :+i
bool msg_OM::Execute( const xParameters& Param )
{
	//elog << "Entered msg_OM" << std::endl;
	//elog << Param << endl;

if( Param.size() < 3 )
	{
	elog	<< "msg_OM> Invalid number of arguments"
			<< endl ;
	return false ;
	}
...
if( (NULL == clientSource) && (NULL == serverSource) )
	{
	elog	<< "msg_OM> Unable to find source: "
			<< Param[ 0 ]
			<< endl ;
	return false ;
	}
...
if( NULL == theChan )
	{
	elog	<< "msg_OM> Unable to find channel: "
		<< Param[ 1 ]
		<< endl ;
	return false ;
	}
...
/*
// OAD M ripper_ :+owg
bool msg_OM::onUserModeChange( const xParameters& Param )
{
...
if( NULL == theClient )
	{
	elog	<< "msg_OM::OnUserModeChange> Unable to find target "
		<< "client: "
		<< Param[ 1 ]
		<< endl ;
	return false ;
...
if( theClient->getNickName() != Param[ 1 ] )
	{
	elog	<< "msg_OM::OnUserModeChange> User trying to change "
