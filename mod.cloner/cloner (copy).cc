...
if( command == "SHOWCOMMANDS" )
	...
	if( topic == "SHOWCOMMANDS" )
		...
		{
		Notice( theClient, "%s <#channel> - Make cloner "
...
//		<< endl ;
	...
	Join(chanName);
	MyUplink->RegisterChannelEvent(chanName.c_str(),this);
	}
...
void cloner::OnChannelMessage(iClient* theClient, Channel* theChan, const string& Message)
{
	elog << "cloner::OnChannelMessage> " << Message << endl ;
	OnPrivateMessage(theClient,Message,false);
	return xClient::OnChannelMessage(theClient,theChan,Message);
}

void cloner::OnFakeChannelMessage(iClient* theClient, Channel* theChan, const string& Message)
{
	elog << "cloner::OnChannelMessage> " << Message << endl ;
	OnPrivateMessage(theClient,Message,false);
	return xClient::OnFakeChannelMessage(theClient,this->getInstance(),theChan,Message);
}
