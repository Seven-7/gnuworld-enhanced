 ...
 * from the bot.
 *
 * Added NOFORCE, insufficient access for AUTOVOICE an AUTOOP responses - Seven <gergo_f@yahoo.com>
 *
 ...
 * USA.
 *
 * $Id: responses.h,v 1.3 2012/05/13 01:25:52 Seven Exp $
 */

#ifndef __RESPONSES_H
#define __RESPONSES_H "$Id: responses.h,v 1.3 2012/05/13 01:25:52 Seven Exp $"
...
namespace gnuworld
	...
	namespace language
	{
		...
		const int chan_noforce =			179;
		const int insuff_avoice =			180;
		const int insuff_aop =			181;
		const int reason_must =			182;
		const int susp_reason =			183;
		const int unsusp_reason =			184;
		const int have_notes =			185;
		const int noaccpt_notes =			186;
		const int exc_max_notes =			187;
		const int note_sended =			188;
		const int read_note_help =			189;
		const int have_nonotes =			190;
		const int message_id =			191;
		const int erase_note_help =			192;
		const int all_notes_erased =			193;
		const int inv_messageid =			194;
		const int erased_messageid =			195;
		const int acc_susp =			196;
		const int acc_unsusp =			197;
		const int acc_new =			198;
		const int acc_modif =			199;
		const int acc_rem =			200;
		const int userurl_on =			201;
