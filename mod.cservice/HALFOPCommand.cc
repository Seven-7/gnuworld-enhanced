 ...
 * HALFOPCommand.cc
 *
 * HALFOP's one or more users on a channel the user as access on.
 *
 ...
 * USA.
 *
 * $Id: HALFOPCommand.cc,v 1.0 2016/02/18 08:37:52 Seven Exp $
 */
...
bool HALFOPCommand::Exec( iClient* theClient, const string& Message )
{
bot->incStat("COMMANDS.HALFOP");
...
vector< iClient* > halfopList; // List of clients to halfop.

if( st.size() < 3 ) /* No nicks provided, assume we halfop ourself. :) */
	{
	halfopList.push_back(theClient);
	source = Message;
	delim = ' ';
	}
else
...
for( ; counter < st2.size() ; ++counter )
	...
	// User is already halfopped? - Calling 'opping' in response is just fine	--Seven
	...
	/*
	 *  If the channel has the STRICTOP flag set, we are only allowed to halfop people who
	...
	if (theChan->getFlag(sqlChannel::F_STRICTOP))
		...
		else if (!(bot->getEffectiveAccessLevel(authUser, theChan, false) >= level::halfop))
			{
			bot->Notice(theClient,
				bot->getResponse(theUser,
	...
	if( tmpBan && (tmpBan->getLevel() <= 75) )
		...
		bot->Notice(theClient,
			"%s isn't allowed to be halfopped on %s",
